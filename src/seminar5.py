...
def build_padded_conv_layer(kernel_size) -> tf.keras.layers.Conv2D:
    ...
    my_layer = tf.keras.layers.Conv2D(2, kernel_size, padding ='same' )
    return my_layer
...
def build_pooling_layer() -> tf.keras.layers.MaxPooling2D:
    ...
    my_layer = tf.keras.layers.MaxPooling2D((2, 2), 2)
    return my_layer
...
def draw_weights(net_path):
    ...
    for i in range(64):
        ...
        plt.axis('off')
    plt.savefig('/Users/alfa/PycharmProjects/neuralnets/output/seminar5/weights.png')
